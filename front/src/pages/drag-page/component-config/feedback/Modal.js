...
export default {
    ...
    hooks: {
        beforeAdd: options => {
            ...
            const handleModalCancel = `handleModalCancel__${uid}`;
            const handleModalShow = `handleModalShow__${uid}`;
            node.__config = {
                pageState: {
                    [field]: false,
                },
                pageFunction: {
                    [handleModalShow]: `() => {setState({${field}: true})}`,
                    [handleModalCancel]: `() => {setState({${field}: false})}`,
                },
            };
